...
- **Backend**: Laravel 13, PHP 8.5, Sanctum SPA auth
- **Frontend**: Vue 3, Inertia.js v3, Tailwind CSS v4 (CSS-first config, no `tailwind.config.js`), shadcn-vue (new-york-v4), Lucide icons (`@lucide/vue`), Wayfinder typed routes
...
All commands run inside the lerd FPM container via `exec` tool, or on the host where noted.

```
# PHP quality (run inside container via exec)
composer lint          # Pint (PSR-12, laravel preset)
composer lint:check    # Pint dry-run
composer types:check   # PHPStan level 7
composer test          # clears config, lint:check, types:check, then Pest

# Frontend quality (run on host via fnm node at .node-version)
...
Always run in this order: **lint → typecheck → test**. Each step depends on the previous passing.

- `composer lint:check` (Pint) — PHP style
- `composer types:check` (PHPStan level 7) — PHP static analysis
- `node node_modules/.bin/vue-tsc --noEmit` — TypeScript/Vue type check
- `node node_modules/.bin/eslint resources/js/` — JS/Vue lint
- `composer test` (Pest) — runs all three checks then tests
...
- **Tailwind v4**: no `tailwind.config.js`. Theme tokens live in `resources/css/app.css` via `@theme inline` and HSL CSS variables. Dark mode uses `.dark` class (not media query)
- **shadcn-vue components**: in `resources/js/components/ui/`. Use `cn()` from `@/lib/utils` for class merging (clsx + tailwind-merge)
- **Prettier**: 4-space indent, single quotes, no semicolons (via `.prettierrc`), Tailwind class sorting via `prettier-plugin-tailwindcss`
...
- **Inertia layouts**: selected in `resources/js/app.ts` by page name. Welcome = null (standalone), default = AppLayout (sidebar), auth/ = AuthLayout, settings/ = [AppLayout, SettingsLayout]
...
Follow `.claude/rules/changelog.md` when updating `CHANGELOG.md`. Keep a Changelog 1.1.0 format, SemVer, entries in imperative mood.
