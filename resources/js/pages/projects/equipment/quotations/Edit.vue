...
const form = useForm({
    supplier_name: props.quotation.supplier_name,
    quoted_cost: String(props.quotation.quoted_cost ?? ''),
    quoted_lead_time_days: props.quotation.quoted_lead_time_days
        ? String(props.quotation.quoted_lead_time_days)
        : '',
    quote_date: props.quotation.quote_date ?? '',
    status: props.quotation.status,
});

const submit = () => {
    form.patch(
        `/projects/${props.project.id}/equipment/${props.equipment.id}/quotations/${props.quotation.id}`,
    );
};
...
<template>
    <Head :title="`Edit Quotation - ${quotation.supplier_name}`" />
    ...
    <div class="flex h-full flex-1 flex-col gap-6 overflow-x-auto rounded-xl p-4">
        <div class="flex items-center gap-4">
            ...
            <Heading
                ...
                :description="`${quotation.supplier_name}`"
            />
        </div>
